...
if uploaded_file is not None:
    try:
        ...
        if "Grant Number" in df.columns:
            df = df[df["Grant Number"].astype(str).str.isdigit()]  # Only true grants
        else:
            df = df[~df["Last Name"].astype(str).str.lower().str.contains("total")]
        ...
        AE_sum = AE.sum() if AE.sum() != 0 else 1
        AH_sum = AH.sum() if AH.sum() != 0 else 1

        # --- RESULTS ---
        results = {
            "Weighted Average Exercise Price - Outstanding": (EP * (AE / AE_sum)).sum(),
            "Weighted Average Exercise Price - Exercisable": (EP * (AH / AH_sum)).sum(),
            "Weighted Average Remaining Contractual Life - Outstanding": (O * (AE / AE_sum)).sum(),
            "Weighted Average Remaining Contractual Life - Exercisable": (O * (AH / AH_sum)).sum(),
